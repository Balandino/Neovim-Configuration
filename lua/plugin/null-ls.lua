-- local syluaConfig
-- if vim.fn.has('win64') == 1 then
--    syluaConfig = 'C:/Lua_Formatter/stylua.toml'
-- else
--    syluaConfig = '/home/mkg/coding/lua_formatter/stylua.toml'
-- end

require('null-ls').setup({
   debug = false,
   sources = {
      require('null-ls').builtins.formatting.clang_format.with({
         disabled_filetypes = { 'java', 'cpp' },
         filetypes = { 'c', 'cs' },
      }),
      -- require('null-ls').builtins.formatting.stylua.with({
      --    extra_args = { '--config-path', syluaConfig },
      -- }),
      require('null-ls').builtins.diagnostics.selene.with({
         filetypes = { 'lua', 'luau' }
      }),
      require('null-ls').builtins.formatting.prettierd.with({
         filetypes = { 'css' },
      }),
      require('null-ls').builtins.formatting.black.with({
         filetypes = { 'python' },
      }),
      require('null-ls').builtins.diagnostics.vulture.with({
         filetypes = { 'python' },
         -- https://pypi.org/project/vulture/
         -- 60% was causing problems
         extra_args = { '--min-confidence 61' },
      }),
      require('null-ls').builtins.diagnostics.pylint.with({
         filetypes = { 'python' },
         -- https://pylint.pycqa.org/en/1.7/user_guide/options.html
         -- extra_args = { '--errors-only' },
         extra_args = { '--disable=missing-docstring', '--disable=line-too-long', '--disable=too-many-arguments' },
         -- command = { 'C:\\Coding\\Workpad\\Python\\TesterVirtual\\Scripts\\pylint.exe' },
         only_local = { '.\\Scripts\\pylint.exe' },
      }),
      require('null-ls').builtins.formatting.isort.with({
         filetypes = { 'python' },
      }),
   },
})
